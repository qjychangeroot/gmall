...
public class SearchServiceImpl implements SearchService {
    ...
    private SearchResponse parseResult(SearchResult result) {
        ...
        if (!CollectionUtils.isEmpty(cateBuckets)){
            SearchResponseAttrVO categotyVO = new SearchResponseAttrVO();
            categotyVO.setName("分类");
            List<String> categoryValues = cateBuckets.stream().map(cateBucket -> {
                Map<String,Object> map = new HashMap<>();
                map.put("id",cateBucket.getKeyAsString());
                TermsAggregation categoryNameAgg = cateBucket.getTermsAggregation("categoryNameAgg");
                map.put("name",categoryNameAgg.getBuckets().get(0).getKeyAsString());
                return JSON.toJSONString(map);
            }).collect(Collectors.toList());
            categotyVO.setValue(categoryValues);
            response.setCatelog(null);
        }
        //解析搜索属性的聚合结果集
